...
from amazon_emr_serverless_image_cli.helper.docker_util.docker_cl import DockerCommand
from amazon_emr_serverless_image_cli.helper.docker_util.docker_helper import verify_docker
...
class TestDockers(unittest.TestCase):
    ...
    def test_docker_inspect(self):
        self.docker_cmd.client.api.inspect_image = mock.Mock(
            return_value=dict())
    ...
    def test_docker_run(self):
        self.docker_cmd.create_container = mock.Mock(
            return_value=docker.models.containers.Container())
        self.docker_cmd.container = self.docker_cmd.create_container()
        self.docker_cmd.container.exec_run = \
            mock.Mock(return_value=docker.models.containers.ExecResult(
                exit_code=0, output=b'something'))
        actual = self.docker_cmd.docker_run("image", "command")
        self.assertEqual(actual, docker.models.containers.ExecResult(
            exit_code=0, output=b'something'))
