...
from amazon_emr_serverless_image_cli.validation_tool.validation_models.validation_models import \
    FileStructure
from amazon_emr_serverless_image_cli.validation_tool.validation_tests.check_files import \
    CheckFiles
...
class TestCheckFileStructure(unittest.TestCase):
    ...
    def test_match(self, docker_constructor):
        ...
        logger = self.log
        docker_cmd.docker_list_files = mock.MagicMock(
            name="match", return_value=b'something\ntests\nhere\n')
        file_structure_list = ['Test1']
        file_structure = [FileStructure(
            'Test1', '/usr/bin', ['asdfasdf', 'sdfasdf'], None)]
        file_check_instance = CheckFiles(
            file_structure_list, file_structure, docker_cmd, 'image_uri', [], [], logger)
    ...
    @mock.patch('amazon_emr_serverless_image_cli.validation_tool.validation_tests.check_files.CheckFiles.match')
