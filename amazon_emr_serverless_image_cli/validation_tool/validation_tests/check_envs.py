...
class CheckEnvs(base_check.BaseCheck):
    ...
    def match(self, env_variables):
        ...
        for key in self.env_list:
            ...
            if env.env_name in env_variables and env.env_value is not None and env.env_value != env_variables[env.env_name]:
                self.log.error("%s MUST set to %s : FAIL" % (env.env_name, env.env_value))
                environment_test = False
            elif env.env_name not in env_variables or len(env_variables[env.env_name]) == 0:
                self.log.error("%s MUST be set : FAIL" % env.env_name)
                environment_test = False
            else:
                self.log.info("%s is set with value: %s : PASS" % (env.env_name, env_variables[env.env_name]))
