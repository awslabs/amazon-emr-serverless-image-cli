from amazon_emr_serverless_image_cli.validation_tool import check_inputs
from amazon_emr_serverless_image_cli.validation_tool.validation_tests import (check_envs, check_files, check_local_job_run,
                                                                              check_manifest)
...
def validate_all(inspect_result,
    ...
    all_tests = [check_manifest.CheckManifest(inspect_result, image.manifest_config, log),
                 ...
                 check_files.CheckFiles(image.file_structures, file_structure, docker_cmd, docker_image_uri, env_vars,
                                        inspect_result['Config']['Env'], log)]

    # Only spark has local job runs
    if image_type == 'spark':
        all_tests.append(check_local_job_run.CheckLocalJobRun(docker_image_uri, docker_cmd, log))
