...
class FileStructure(yaml.YAMLObject):
    ...
    def __init__(self, name, relative_location, file_prefixes, env_var):
        self.name = name
    ...
    def from_yaml(cls, loader, node):
        ...
        if 'EnvironmentVariableRootLocation' in data:
            env_var = data['EnvironmentVariableRootLocation']
        else:
            env_var = None

        return cls(name, relative_location, file_prefixes, env_var)
